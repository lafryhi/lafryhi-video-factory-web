...
export function PreviewPanel(props: Props) {
  ...
  }, [imageUrl, props.scene?.sceneId, localTime, props.playing, props.scene?.sourceAudio]);
  ...
  return <main className="preview-panel panel">
    ...
    <div ref={stage} data-testid="preview-stage" className={`stage ${props.project.videoFormat === "vertical_9_16" ? "vertical" : "landscape"}`}>
      {imageUrl ? <div className="live-scene" key={props.scene?.sceneId}>{props.scene?.mediaType === "video" ? <video ref={clipVideo} src={imageUrl} playsInline preload="auto" muted={props.scene?.sourceAudio === false} style={motionStyle(props.scene, props.sceneProgress, props.project.fillMode)} onLoadedMetadata={e => { e.currentTarget.muted = props.scene?.sourceAudio === false; e.currentTarget.volume = 1; e.currentTarget.currentTime = (props.scene?.sourceStartSeconds || 0) + localTime; if (props.playing) void e.currentTarget.play().catch(() => {}); }}/> : <img data-testid="preview-image" src={imageUrl} alt="Current scene" style={motionStyle(props.scene, props.sceneProgress, props.project.fillMode)}/>}{props.scene?.texts?.filter((overlay) => localTime >= overlay.startSeconds && (overlay.endSeconds === null || localTime <= overlay.endSeconds)).map((overlay) => <div key={overlay.id} className="preview-text" dir="auto" style={{ left: `${overlay.x}%`, top: `${overlay.y}%`, color: overlay.color, fontFamily: overlay.fontFamily, fontSize: `${Math.max(10, overlay.fontSize / 2)}px` }}>{overlay.text}</div>)}</div> : <div className="stage-empty"><Play/><strong>Your preview appears here</strong><span>Drop media into the media library to begin.</span></div>}
